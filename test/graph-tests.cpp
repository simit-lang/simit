...
TEST(Set, AddAndGetFromTwoFields) {
  ...
  ASSERT_FLOAT_EQ(101.1, f2.get(i));
}
...
TEST(Field, Vector) {
  ...
  ASSERT_FLOAT_EQ(1u, vec1.getOrder());
  vec1(0) = 1.1;
  vec1(1) = 2.2;
  vec1(2) = 3.3;
  ...
  ASSERT_FLOAT_EQ(4.4, vec4(0));
  ASSERT_FLOAT_EQ(5.5, vec4(1));
  ASSERT_FLOAT_EQ(6.6, vec4(2));
}
...
TEST(Field, Matrix) {
  ...
  ASSERT_FLOAT_EQ(1.1, mat3(0,1));
  ASSERT_FLOAT_EQ(2.2, mat3(1,0));
  ASSERT_FLOAT_EQ(3.3, mat3(2,1));

  TensorRef<simit_float,3,2> mat4 = x.get(p1);
  ASSERT_FLOAT_EQ(4.4, mat4(0,0));
  ASSERT_FLOAT_EQ(5.5, mat4(1,1));
  ASSERT_FLOAT_EQ(6.6, mat4(1,0));
}
...
TEST(Field, Tensor) {
  ...
  ASSERT_FLOAT_EQ(3u, t1.getOrder());
  t1(0,1,3) = 1.1;
  t1(1,0,2) = 2.2;
  t1(2,1,0) = 3.3;
  ...
  ASSERT_FLOAT_EQ(1.1, t3(0,1,3));
  ASSERT_FLOAT_EQ(2.2, t3(1,0,2));
  ASSERT_FLOAT_EQ(3.3, t3(2,1,0));

  TensorRef<simit_float,2,3,4> t4 = x.get(p1);
  ASSERT_FLOAT_EQ(4.4, t4(0,0,1));
  ASSERT_FLOAT_EQ(5.5, t4(1,1,3));
  ASSERT_FLOAT_EQ(6.6, t4(1,0,0));
}
...
TEST(EdgeSet, CreateAndGetEdge) {
  ...
  ASSERT_FLOAT_EQ(x.get(edges.getEndpoint(e,0)), 1.1);
  ASSERT_FLOAT_EQ(x.get(edges.getEndpoint(e,1)), 3.1);
...
TEST(EdgeSet, EdgeIteratorTest) {
  ...
  for (auto iter=edges.endpoints_begin(e1);
       ...
       iter++) {
    if (count==0)
      ASSERT_FLOAT_EQ(x.get(*iter), 1.1);
    if (count==1)
      ASSERT_FLOAT_EQ(x.get(*iter), 3.1);
    count++;
  }
