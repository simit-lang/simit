...
// Function prototypes
static void DumpDomainToVisit(DBfile *db, int iter, double time,
                              int Xsize, int Ysize, Set *quads, Set *points);

#endif

/**********************************************************************/
void DumpToVisit(std::string ZoneName,int iter, double time,
                 int Xsize, int Ysize, Set *quads, Set *points)
{
#ifdef VIZ_MESH
  char subdirName[32];
...
static void
DumpDomainToVisit(DBfile *db, int iter, double time,
                  int Xsize, int Ysize, Set *quads, Set *points)
{
  int ok = 0;
  ...
  for (auto quad = quads->begin(); quad != quads->end(); ++quad) {
    for (auto point = quads->getEndpoints(*quad).begin();
        point != quads->getEndpoints(*quad).end(); ++point) {
      conn[ci] =  point->getIdent() ;
      ci++;
    }
    qi++;
  }
  ok += DBPutZonelist2(db, "connectivity", quads->getSize(), 2, conn,
                       quads->getSize()*4,0,0,0, shapetype, shapesize,
                       shapecnt, 1, NULL);
