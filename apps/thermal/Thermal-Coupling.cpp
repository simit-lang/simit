...
int main(int argc, char **argv)
{
  ...
  //3 - Read the Pan part
  Thermal Pan = Thermal(PM.get(TPM::PanFileName),
                        PM.get(TPM::CGNSFileName),"Pan",1);

  //4 - Read the Steak part
  Thermal Steak = Thermal(PM.get(TPM::SteakFileName),
                          PM.get(TPM::CGNSFileName),"Steak",2);
  ...
  while ((time < PM.get(TPM::timeMax)) && (iter<PM.get(TPM::iterMax))) {
    ...
    if (converged)
      std::cout << " >>> Strong coupling converged in "
                << iter_coupling << " iterations" << std::endl;
    time+=dt;
