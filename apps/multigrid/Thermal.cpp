...
Thermal::Thermal(std::string paramFile, std::string CGNSFileName_L0, std::string CGNSFileName_L1, std::string zoneName, int index_zone)
{
	...
	for(int i=0; i<2; i++) {
		...
		if (cg_open(filenames[i].c_str(),CG_MODE_READ,&index_file)) cg_error_exit();
