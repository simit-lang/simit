// Modified from https://github.com/dropbox/pyston/blob/master/src/codegen/dis.cpp
...
DisassemblerJITEventListener::DisassemblerJITEventListener() {
    ...
    const llvm::StringRef triple = llvm::sys::getDefaultTargetTriple();
    const llvm::StringRef CPU = llvm::sys::getHostCPUName();
    llvm::TargetMachine* targetmachine = target->createTargetMachine(triple, CPU, "", options);
    ...
    bool verbose = false;
#if LLVMREV < 208205
    llvm::MCStreamer* streamer = target->createAsmStreamer(*Ctx, llvm::ferrs(), verbose, true, true, true, IP, CE, TAB, true);
#else
    ...
    asm_printer = target->createAsmPrinter(*targetmachine, *streamer);
    assert(asm_printer);
    ...
    asm_printer->Mang = new llvm::Mangler(tmachine);
